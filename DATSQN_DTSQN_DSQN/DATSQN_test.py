...
LOG_DIR = './logs/DATSQN_test'

device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

#please enter the game id that you have entered in DATSQN_trian for training
env_id = 'Breakout'
...
class IF(nn.Module):
    ...
    def _base_sub(self, input_):
        return self._base_state_function(input_) - (self.reset >= 0).float() * self.reset * self.vthp - (
                    self.reset < 0).float() * self.reset * self.vthn
