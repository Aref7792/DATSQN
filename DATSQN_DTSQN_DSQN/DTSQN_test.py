...
LOG_DIR='./logs/DTSQN_test'

device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

#please enter the game id that you have entered in DTSQN_trian for training
env_id= 'Breakout'
...
class IF(nn.Module):
    ...
    def reset_mem(self,vmem):
        mem_shift = (vmem >= 0)*(vmem - self.vth)+(vmem < 0)*(vmem + self.vth)
    ...
    def _base_sub(self, input_):
        return self._base_state_function(input_) - (self.reset>=0).float()*self.reset * self.vth - (self.reset<0).float()*self.reset * self.vth
